...
parti_scores <- c("ENS_gagnabilite", 
                  "RN_gagnabilite", "REC_gagnabilite", 
                  "ECO_gagnabilite", "NUP_gagnabilite", "LR_gagnabilite")
...
attribuer_score_gag <- function(row) {
  ...
  if (nuance %in% c("ENS", "LR", "RN", "REC", "ECO", "NUP")) {
    ...
    if (score_col %in% names(row)) {
      ...
      return(NA)  # Gérer le cas où la colonne de score n'existe pas
    }
  } else {
    return(NA)  # Gérer le cas où nuance_candidat n'est pas dans la liste
  }
}
...
df_final2022 <- df_final2022 %>%
  rowwise() %>%
  mutate(score_gagnabilite = attribuer_score_gag(cur_data())) %>%
  ungroup()
...
chemin_fichier <- "/Users/augustajomayans/Library/CloudStorage/GoogleDrive-josephine.mayans@gmail.com/Mon Drive/pessd/data/legislatives/2022/candidats_2022_gag.dta"
